...
def test_advertisement_route_cannot_overwrite_active_connection_route(monkeypatch):
    ...
    # Connecting does not make a cached advertisement a fresh measurement.
    assert device.conn_info["rssi_updated_at"] is None
...
def test_source_name_does_not_fall_back_to_bluetooth_address(monkeypatch):
    import custom_components.fluvalble.core.device as device_module

    device = _make_device()
    device.hass = MagicMock()
    source = "AA:BB:CC:DD:EE:FF"
    for scanner in (None, SimpleNamespace(name=source), SimpleNamespace(name=None)):
        monkeypatch.setattr(device_module.bluetooth, "async_scanner_by_source", lambda *_args: scanner)
        metadata = device._source_metadata(source)
